...
class Symbol:
	...
	def symbol_inspection(self,inspection_tag=""):
		...
		### ?
		if inspection_tag!="":
			self.source_tag = inspection_tag
		else:
			self.source_tag  = self.source
		debug_line = f'{self.source_tag} {self.symbol_name} :symbol_inspection()'
		#print(debug_line,"enter")
		if DEBUGGING:
			print('\n')
		if not self.inspection_lock.locked():
			...
			if self.moo_out:
				...
				if self.moc_order_out==False and self.order_out==False:
					if self.tp_current_shares != 0:
						self.flatten_all()
						self.limit_inspection_block_moc()
				...
				#message(f""""Nothing happe")
				print(self.symbol_name,' moc:',self.moc_out,self.moc_order_out,self.order_out)
			else:
			...
			message(f"""{self.source_tag},{self.symbol_name},Inspection LOCKED""",LOG)
			return 0 
	...
	def limit_inspection_block(self):
		...
		if DEBUGGING:
			for tp_name, tp in list(self.tradingplans.items()):
				debug_line =  f'{self.source_tag} {self.symbol_name} :{tp_name} limit_inspection():'
				limit_request = tp.data['limit_request'].get(self.symbol_name)

				#if limit_request['shares']!=0:
				message(f'{debug_line},{limit_request}',LOG)

	def limit_inspection_block_moc(self):
		"""
		MOC-safe variant: cancel any TP limit orders for this symbol; do not send new ones.
		"""
		for tp_name, tp in list(self.tradingplans.items()):
			...
			# Resolve OID from PID if needed
			if not oid and pid:
				try:
					r = requests.get(f'http://127.0.0.1:5000/papi/{pid}', timeout=0.25)
					d = r.json()
			...
			# Clear LR node so it cannot re-arm
			# lr.update({
			#     'pid': '', 'oid': '', 'status': 'Cancelled',
			#     'target_price': '', 'order_price': '', 'shares': 0,
			#     'fills': {}, 'ts': 0
			# })
			# tp.data['limit_request'][self.symbol_name] = lr
	...
	def status_message(self):
		...
		debug_line =  f'{self.source_tag} {self.symbol_name} :status message'
...
######################################## PHASE 1 fill_check_phase  ###########################################
	...
	def cancel_previous_order(self):
		...
		debug_line = f'{self.source_tag} {self.symbol_name} :cancel_previous_order()'
	...
	def order_update_phase(self):
		...
		debug_line = f'{self.source_tag} {self.symbol_name} :order_update_phase()'
	...
	def fill_check_phase(self):
		...
		debug_line = f'{self.source_tag} {self.symbol_name} :fill_check_phase()'
	...
	def process_fills(self,data):
		...
		#print('DATA:',data)
		debug_line = f'{self.source_tag} {self.symbol_name} :process_fills()'
		...
		message(f"""{debug_line}, Requested: {self.request} new fill {total_new_fills} total fill {total_fills}""",LOG)
	...
	def allocate_shares(self,tp,request_shares, available_fills):
		...
		debug_line = f'{self.source_tag} {self.symbol_name} :allocate_shares()'
...
######################################## PHASE 5 aggragate_phase ########################################
	...
	def adjust_aggresiveness(self):
		...
		debug_line = f'{self.source_tag} {self.symbol_name} :adjust_aggresiveness()'
	...
	def aggragate_phase(self,MOC=False):

		#self.prev_request = self.request
		#self.tp_current_shares,self.expired,self.tp_total_current_shares = self.get_all_current(tps)
		debug_line = f'{self.source_tag} {self.symbol_name} :aggragate_phase()'
		tps = list(self.tradingplans.keys())
		self.central_dispatching_order_request= {}
	...
	def order_pair_off(self,tps,price=0):

		# give t

		debug_line = f'{self.source_tag} {self.symbol_name} :order_pair_off()'
		want = {}
		...
		if price==0:
			...
			for tp,granted_shares in pair_off_results.items():
				...
				if granted_shares>0:
					self.tradingplans[tp].request_fufill(self.symbol_name,granted_shares,price)
				else:
					self.tradingplans[tp].request_fufill(self.symbol_name,granted_shares,price)
...
######################################## PHASE 6  ORDERING PHASE ########################################
	...
	def ordering_phase(self):
		...
		"""
		debug_line = f'{self.source_tag} {self.symbol_name} :ordering_phase()'
...
### EMS API PART ####
	...
	def look_up_orderid(self):
		...
		if self.order_pid!="":
			...
			# if DEBUGGING:
			#   print(self.source_tag,self.symbol_name,':look_up_orderid',data,self.order_id)
...
### REJECTION HANDLING MOUDULE ####
	...
	def rejection_handling(self):
		...
		debug_line = f'{self.source_tag} {self.symbol_name} :rejection_handling(),requested : {self.request}'
	...
	def l1_update_module(self):
		debug_line = f'{self.source_tag} {self.symbol_name} :l1_update_module()'
		try:
