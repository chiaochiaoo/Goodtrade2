...
class Test_MOO_Basics(unittest.TestCase):

	@patch_both_datetimes(datetime(2025, 8, 5, 9, 29, 45))
	def test_moo_no_moo(self, mock_dt_symbol, mock_dt_tp, mock_get):
		state = TestState()
		mock_get.side_effect = lambda url, **kw: dynamic_mock_get(state, url, **kw)
		...
		symbol.time_to_moo("ARCA ACTION ARCX Market MOO DAY")
		symbol.symbol_inspection()
		symbol.symbol_inspection()
		symbol.symbol_inspection()
		...
		self.assertFalse(symbol.moo_out)
		self.assertFalse(symbol.moo_order_out)

		state.order_status = "Filled"
		state.order_id = "moo123"
		state.fill_details = {105.25: -10}
		state.shares = -10
		state.target_share = -10
	...
	@patch_both_datetimes(datetime(2025, 8, 5, 9, 29, 45))
	def test_moo_moo_exit(self, mock_dt_symbol, mock_dt_tp, mock_get):
		state = TestState()
		mock_get.side_effect = lambda url, **kw: dynamic_mock_get(state, url, **kw)
		...
		tp1.set_moo_target("AMD.NQ",0)


		symbol.time_to_moo("ARCA ACTION ARCX Market MOO DAY")

		state.order_id = "Moo_Order"
		state.fill_details = {}
		symbol.symbol_inspection('M1:')

		self.assertTrue(symbol.moo_out)
		self.assertTrue(symbol.moo_order_out)

		#print(tp1.data)
		state.fill_details = {106: -10}
		state.order_status = 'Filled'
		symbol.symbol_inspection('M2:')
		...
		symbol.symbol_inspection('M3:')

		tp1.submit_expected_shares(ticker, 10, False)
		symbol.symbol_inspection('M4:')

		self.assertTrue(symbol.order_out)

	@patch_both_datetimes(datetime(2025, 8, 5, 9, 29, 45))
	def test_moo_moo_entry(self, mock_dt_symbol, mock_dt_tp, mock_get):
		state = TestState()
		mock_get.side_effect = lambda url, **kw: dynamic_mock_get(state, url, **kw)
		...
		self.assertEqual(tp1.data[REALIZED],0)
		...
		tp1.submit_expected_shares(ticker, 20, False)
		symbol.symbol_inspection('M4:')
	...
	@patch_both_datetimes(datetime(2025, 8, 5, 9, 29, 45))
	def test_moo_moo_mix(self, mock_dt_symbol, mock_dt_tp, mock_get):
		state = TestState()
		mock_get.side_effect = lambda url, **kw: dynamic_mock_get(state, url, **kw)
		...
		tp2 = TradingPlan(self, "TP2", {})
		tp2.register_symbol("AMD.NQ", symbol)
		tp2.submit_expected_shares(ticker,25, False)
		symbol.symbol_inspection()
		state.fill_details = {104: 25}
		state.order_status = 'Filled'
		symbol.symbol_inspection()
		...
		self.assertEqual(symbol.tp_current_shares,0)
		self.assertEqual(tp1.data[REALIZED],20)
		self.assertEqual(tp2.data[REALIZED],25)
...
if __name__ == "__main__":
	...
	suite.addTest(Test_MOO_Basics("test_moo_moo_exit"))


	suite.addTest(unittest.makeSuite(Test_MOC_Basics))
	suite.addTest(unittest.makeSuite(Test_MOO_Basics))
	


	suite.addTest(Test_MOO_Basics("test_moo_moo_mix"))
