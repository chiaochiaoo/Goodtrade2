...
from ui_performance_optimizer import (
    UIUpdateThrottler, BatchUpdateManager, MemoryMonitor,
    prioritize_process, configure_tkinter_performance
)
...
class UI:
    def __init__(self, root, manager=None):
        ...
        self.update_throttler = UIUpdateThrottler(min_interval_ms=50)
        self.batch_manager = BatchUpdateManager(root, batch_delay_ms=100)
        self.memory_monitor = MemoryMonitor()
        
        # Apply performance configurations
        configure_tkinter_performance(self.root)
        prioritize_process()
        
        # Track last style update to prevent excessive calls
        self._last_style_update = 0
        self._style_update_cooldown = 200  # ms
    ...
    def update_treeview_styles_throttled(self):
        """Throttled version of style updates to prevent excessive redraws"""
        now = time.time() * 1000
        if now - self._last_style_update < self._style_update_cooldown:
            # Schedule for later
            self.batch_manager.add_to_batch(
                'style_update',
                self._do_style_update
            )
        else:
            self._do_style_update()
    
    def _do_style_update(self):
...
if __name__ == '__main__':
    ...
    app.algo_deployment.add_algo(tp5) 
    app.algo_deployment.add_algo(tp6) 
    app.algo_deployment.add_algo(tp7) 
