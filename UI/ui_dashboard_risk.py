...
class RiskPanel(tb.Frame):
    def __init__(self, parent, master=None, autosave_file: str = None, *args, **kwargs):
        ...
        self.RISK_CFG = {
            "global": {
                ...
                "max_order_size":tk.IntVar(m,10000),
                "max_algo_size":tk.IntVar(m,10000),
        ...
        self.RISK_CFG_TOGGLE = {
            "global": {
                "max_gross_pos": tk.BooleanVar(m, False),
                "max_symbols": tk.BooleanVar(m, False),
                "max_open_orders": tk.BooleanVar(m, False),
                "max_order_size": tk.BooleanVar(m, True),
                "max_algo_size": tk.BooleanVar(m, True),
                "max_order_buying_power": tk.BooleanVar(m, True),
                "daily_unreal_soft_stop": tk.BooleanVar(m, False),
                "daily_unreal_hard_stop": tk.BooleanVar(m, False),
            },
            "per_algo": {
                "max_distinct_algos": tk.BooleanVar(m, False),
                "algo_soft_stop": tk.BooleanVar(m, False),
                "algo_hard_stop": tk.BooleanVar(m, False),
            },
            "per_suffix": {
                ".NQ": {"max_gross_pos": tk.BooleanVar(m, False), "max_symbol_pos": tk.BooleanVar(m, False)},
                ".NY": {"max_gross_pos": tk.BooleanVar(m, False), "max_symbol_pos": tk.BooleanVar(m, False)},
                ".AM": {"max_gross_pos": tk.BooleanVar(m, False), "max_symbol_pos": tk.BooleanVar(m, False)},
                ".EU": {"max_gross_pos": tk.BooleanVar(m, False), "max_symbol_pos": tk.BooleanVar(m, False)},
            },
        }
    ...
    def _validate_number(self, newval: str) -> bool:
        """Validate that input is a number only and >= 100 (or empty for clearing)."""
        if newval == "":
            return True
        try:
            num = float(newval)
    ...
    def _int_spin(self, parent, var, step=1, width=14):
        ...
        return tb.Spinbox(parent, from_=100, to=9_999_999, increment=step, width=width, textvariable=var, 
                         validate="key", validatecommand=vcmd)

    def _dbl_spin(self, parent, var, step=100.0, width=14):
        vcmd = (parent.register(self._validate_number), "%P")
        return tb.Spinbox(parent, from_=100, to=1e9, increment=step, width=width, textvariable=var, 
                         validate="key", validatecommand=vcmd)
    ...
    def _build_ui(self):
        ...
        rows = [
            ...
            ("Max algo size", "max_algo_size", lambda p: self._dbl_spin(p, g["max_algo_size"], 100.0)),
            ("Max per-order size", "max_order_size", lambda p: self._dbl_spin(p, g["max_order_size"], 100.0)),
            ("Max per-order buying power", "max_order_buying_power", lambda p: self._dbl_spin(p, g["max_order_buying_power"], 10000.0)),
        ...
        for i, (label, key, mkspin) in enumerate(rows):
            ...
            # Disable checkbox for unavailable features
            is_available = key in ["max_order_size", "max_algo_size", "max_order_buying_power"]
            cb = tb.Checkbutton(row, variable=tg[key], text="", state="normal" if is_available else "disabled")
            cb.pack(side="left", padx=(4,8))
            tb.Label(row, text=label, width=26, anchor="w", foreground="gray" if not is_available else "").pack(side="left")
        ...
        for suf in [".NQ",".NY",".AM",".EU"]:
            ...
            tb.Label(row, text=suf, width=8, foreground="gray").pack(side="left", padx=4)
            cbg = tb.Checkbutton(row, variable=self.RISK_CFG_TOGGLE["per_suffix"][suf]["max_gross_pos"], text="", state="disabled"); cbg.pack(side="left")
            ...
            cbs = tb.Checkbutton(row, variable=self.RISK_CFG_TOGGLE["per_suffix"][suf]["max_symbol_pos"], text="", state="disabled"); cbs.pack(side="left")
        ...
        for i, (label, key, mkspin) in enumerate(rows):
            ...
            cb = tb.Checkbutton(row, variable=ta[key], text="", state="disabled")
            cb.pack(side="left", padx=(4,8))
            tb.Label(row, text=label, width=20, anchor="w", foreground="gray").pack(side="left")
    ...
    def effective_global(self,key,default=None):
        try:
            ...
            return 1000 if default is None else default
