...
class TFMPanel(tb.Frame):
    ...
    def _cap_shares_to_max_size(self, shares):
        """Cap shares to max_algo_size limit if configured."""
        try:
            max_size = self.ui.dashboard.risk_panel.effective_global("max_algo_size")
            if max_size and int(shares) > max_size:
                return max_size
        except:
            pass
        return int(shares)
    ...
    def _noop_submit(self):
        ...
        try:
            ...
            share  = self._cap_shares_to_max_size(self.var_shares.get())
